...
int find_slot(Table& table, const std::string& key) {
    ...
    for (std::size_t step = 0; step < table.size(); step++) {
        std::size_t index = (start + step * step) & (table.size() - 1);
...
int find_insert_slot(Table& table, const std::string& key) {
    ...
    for (std::size_t step = 0; step < table.size(); step++) {
        std::size_t index = (start + step * step) % table.size();
...
void resize_table(Table& table, std::size_t new_size) {
    ...
    for (const Entry& entry : table) {
        if (entry.state == State::OCCUPIED) {
            ...
            if (index == -1) {
                std::cerr << "Resize failed: no slot found\n";
                return;
            }
...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (is_present) {
            ...
            if (insert_index == -1) {
                std::cerr << "Insertion failed: no slot found\n";
                return 1;
            }
